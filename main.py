...
def train(layer1:int, layer2:int, lr_max:float, lr_min:float, l2:float, lr_delta:float, epoch_num:int=30):
    with open('log_main.txt', 'w', encoding='utf-8') as f:
        # 模型定义
        ...
        for epoch in range(epoch_num):
            ...
            print(f'[{epoch}/{epoch_num}] loss={result.loss} loss_test={result.loss_test} acc={acc}')
            f.write(f'[{epoch}/{epoch_num}] loss={result.loss} loss_test={result.loss_test} acc={acc}\n')
            if acc > 0.8:
                trainer.start_decrease = True
        # 测试评估
        return max_acc

train(512, 256, 1, 1e-5, 1e-4, 0.95, 10000)